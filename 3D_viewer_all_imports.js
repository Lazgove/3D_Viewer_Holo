...
function stopLoading() {
  ...
  // Function to animate progress to a specified target
  function updateProgressTo(target) {
    // Ensure target is within valid range
    target = Math.min(target, 100);
    
    function animateProgress() {
        if (progress < target) {
            progress++; // Increment progress smoothly
            ...
            progressCircle.style.strokeDashoffset = offset; // Update circle's offset
            requestAnimationFrame(animateProgress); // Continue animation to the target
        } else if (progress >= 100) {
            // Hide loading container when progress is complete
            loadingContainer.style.display = 'none';
        }
    }
